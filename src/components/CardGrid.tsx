...
const staticCards: Card[] = [
  {
    ...
    description:
      "يوفر تحليلاً تفصيلياً للعروض في المناقصات الحكومية وتطبيق آليات التفضيل للمنشآت الوطنية وفقاً للأنظمة الرسمية.",
    badge: "خبير",
    preview_url:
  ...
  },
  {
    ...
    description:
      "يوفر تحليلاً دقيقاً للمنتجات الوطنية في القائمة الإلزامية ومساعدة شاملة في متطلبات القائمة الإلزامية والاستثناءات.",
    badge: "خبير",
    preview_url:
      "https://images.unsplash.com/photo-1450101499163-c8848c66ca85?w=800&h=400&fit=crop&q=80",
  ...
  },
  {
    ...
    description:
      "نظام شامل ينظم متطلبات المحتوى المحلي في المشتريات الحكومية والمنافسات، يتضمن آليات التطبيق والمعايير المطلوبة لتحقيق أهداف المحتوى المحلي في القطاع الحكومي.",
    badge: "نظام",
    preview_url:
...
const CardGrid = ({ activeFilter }: CardGridProps) => {
  const [allCards, setAllCards] = useState<Card[]>(staticCards);
  ...
  useEffect(() => {
    const fetchCards = async () => {
      try {
        ...
        setError(
          err instanceof Error ? err.message : "حدث خطأ في تحميل الكاردات"
        );
  ...
  const filteredCards =
    activeFilter === "الكل"
      ...
      : allCards.filter((card) => {
          const cardType = card.type || card.badge;
